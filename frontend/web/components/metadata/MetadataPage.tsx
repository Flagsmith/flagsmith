        ...
        <PanelSearch
          ...
          header={
            <Row className='table-header'>
              ...
              <div className='table-column' style={{ width: metadataWidth[0] }}>
                Environment
              </div>
              <div className='table-column' style={{ width: metadataWidth[1] }}>
                Segment
              </div>
              <div className='table-column' style={{ width: metadataWidth[2] }}>
                Feature
              </div>
          ...
          renderRow={(metadata: MergeMetadata) => (
            ...
            >
              ...
              >
                <div className='table-column d-flex text-center'>
                  {metadata.content_type_fields.find(
                    (m) => m.content_type === environmentContentType?.id,
                  ) ? (
                    <>
                      {metadata.content_type_fields.some(
                        (field) =>
                          field.content_type === environmentContentType?.id,
                      ) ? (
                        <>
                          {metadata.content_type_fields.some(
                            (field) =>
                              ...
                              field.is_required_for.length > 0,
                          ) ? (
                            <>
                        ...
                        <>
                          <Icon name='close-circle' width={20} />
                        </>
                      )}
                    </>
                  ) : (
              ...
              <div className='table-column' style={{ width: '150px' }}>
                <div className='table-column' style={{ width: '150px' }}>
                  {metadata.content_type_fields.find(
                    (m) => m.content_type === segmentContentType?.id,
                  ) ? (
                    <>
                      {metadata.content_type_fields.some(
                        (field) =>
                          field.content_type === segmentContentType?.id,
                      ) ? (
                        <>
                          {metadata.content_type_fields.some(
                            (field) =>
              ...
              >
                ...
                >
                  {metadata.content_type_fields.find(
                    (m) => m.content_type === featureContentType?.id,
                  ) ? (
                    <>
                      {metadata.content_type_fields.some(
                        (field) =>
                          field.content_type === featureContentType?.id,
                      ) ? (
                        <>
                          {metadata.content_type_fields.some(
                            (field) =>
