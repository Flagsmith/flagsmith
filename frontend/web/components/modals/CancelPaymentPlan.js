...
const CancelPaymentPlanModal = class extends Component {
  ...
  sendCancellation = () => {
    ...
    const url =
      'https://post.formlyapp.com/bullet-train-payment-plan-cancellation'
    const options = {
      body: JSON.stringify({
        cancellationReason,
        orgId: org.id,
    ...
    fetch(url, options).then((res) => {
      ...
      if (isSuccess) {
        AppActions.editOrganisation(
          Object.assign({}, org, { pending_cancellation: true }),
        )
      }
    })
  }
  ...
  render() {
    ...
    return (
      <div>
        <div>
          You will continue being able to use Flagsmith till the end of your
          plan even after cancelling
        </div>
        <FormGroup>
          <div>
            <strong>
          ...
          <textarea
            ...
            onChange={(e) =>
              this.setState({
                cancellationReason: Utils.safeParseEventValue(e),
              })
            }
          />
        </FormGroup>
