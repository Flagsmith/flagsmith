...
const CreateProject = ({ history, onSave }) => {
  ...
  const assignProjectAdmins = async (projectId) => {
    if (!adminIds.length) return
    await Promise.all(
      adminIds.map((userId) =>
        _data.post(`${Project.api}projects/${projectId}/user-permissions/`, {
          admin: true,
          permissions: [],
          user: userId,
        }),
  ...
  const close = async (data = {}) => {
    ...
    if (projectId && adminIds.length) {
      ...
      try {
        ...
        // Surface but don't block navigation — project itself was created.
