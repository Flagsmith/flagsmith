...
const ChangeRequestsPage = class extends Component {
    ...
    render() {
        ...
        const has4EyesPlan = Utils.getPlansPermission('4_EYES');
                    ...
                    {(
                        <div>
                            ...
                            >
                                        ...
                                        <>
                                            <p className='text-center mt-2'>
                                                {environment && !Utils.changeRequestsEnabled(environment.minimum_change_request_approvals) ? (
                                                    <InfoMessage>
                                                        You currently have change requests disabled To enable this
                                                        feature set a minimum number of approvals in <Link
                                                        to={`/project/${projectId}/environment/${environmentId}/settings`}>Environment
                                                        Settings</Link>
                                                    </InfoMessage>
                                                ) : (
                                                    <div>
                                                                ...
                                                                <div>
                                                                    <ButtonLink>
                                                                        {title}
                                                                        {
                                                                            isScheduled && (
                                                                                <span
                                                                                    className='ml-1 mr-4 ion ion-md-time' />
                                                                            )
                                                                        }
                                                                    </ButtonLink>
                                                                    <div className='list-item-footer faint'>
                                                                        Created
                                                                        at {moment(created_at).format('Do MMM YYYY HH:mma')} by {user && user.first_name} {user && user.last_name}
                                                                    </div>
                                                                </div>
                                                            </Row>
                                                        </Link>
                                ...
                                <TabItem
                                    tabLabel={`Scheduled${dataScheduledPaging ? ` (${dataScheduledPaging.count})` : ''}`}>
                                    {!hasSchedulePlan ? (
                                        <div className="mt-2">
                                        <InfoMessage>
                                            View and manage your feature changes with a Change Request flow with our <a
                                ...
                                <TabItem tabLabel={`Closed${dataClosedPaging ? ` (${dataClosedPaging.count})` : ''}`}>
                                    <PanelSearch
                                        ...
                                        renderFooter={() => <JSONReference className='mt-4' title={'Change Requests'}
                                                                           json={dataClosed} />}
                                        renderRow={({ title, user: _user, created_at, id }, index) => {
                                            const user = OrganisationStore.model && OrganisationStore.model.users.find(v => v.id === _user);
                                            return (
                                                <Link
                                                    ...
                                                    <Row className='list-item clickable'>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className='list-item-footer faint'>
                                                                Live
                                                                from {moment(created_at).format('Do MMM YYYY HH:mma')} by {user && user.first_name} {user && user.last_name}
                                                            </div>
                                                        </div>
                                                    </Row>
                                                </Link>
