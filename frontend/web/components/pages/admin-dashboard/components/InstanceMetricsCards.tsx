  ...
  return (
    ...
    >
      <StatItem
        ...
        sub={`${summary.active_organisations} active in last ${days} days`}
      />
      <StatItem
        icon='features'
        label='Total Flags'
        value={summary.total_flags}
        sub={`Across ${summary.total_projects} projects`}
      />
      <StatItem
        icon='people'
        label='Seats'
        value={summary.total_users}
        sub={`${summary.active_users} active of ${summary.total_users}`}
      />
      <StatItem
        icon='bar-chart'
        label='API Calls'
        value={summary.total_api_calls_30d}
        sub={`Across ${summary.total_environments} environments`}
      />
      <StatItem
        icon='options-2'
        label='Integrations'
        value={summary.total_integrations}
        sub={`Across ${summary.total_organisations} organisations`}
      />
    </div>
  )
}
