    ...
    render = () => {
        ...
        if ((!isSignup || !disableOauthRegister) && !disableSignup) {
            ...
            if (Utils.getFlagsmithValue('oauth_google')) {
                oauths.push((
                    <GoogleLogin
                        clientId={JSON.parse(Utils.getFlagsmithValue('oauth_google')).clientId}
                        onSuccess={(e)=>{
                            document.location = `${document.location.origin}/oauth/google?code=${e.accessToken}`;
                        }}
                        render={(renderProps)=>(
                            <a
                                key="google" className="btn btn__oauth btn__oauth--google"
