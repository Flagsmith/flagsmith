...
const AddEditTags: FC<AddEditTagsType> = ({projectId, readOnly, value, onChange}) => {
    ...
    const filteredTags = useMemo(() => {
        ...
        if (_filter) {
            return loFilter(projectTags, tag => tag.label.toLowerCase().includes(filter));
        }
        return projectTags||[];
    }, [filter, projectTags])
                            ...
                            {filteredTags && filteredTags.map((tag, index) => (
                                <div key={tag.id}>
                                    <Row className="py-2">
                                        <Flex>
                                            <Tag
                                                className="px-2 py-2" onClick={selectTag}
