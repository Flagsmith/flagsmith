var Utils = {
    ...
    getPaging(currentPage, totalPages, rangeAround) { // provides paging information based on properties
        totalPages = Math.max(totalPages, currentPage);
        const around = (rangeAround - 1) / 2;


        let startPage = Math.min(totalPages - rangeAround + 1, Math.max(1, currentPage - around));


        const endPage = Math.min(totalPages + 1, startPage + rangeAround);
