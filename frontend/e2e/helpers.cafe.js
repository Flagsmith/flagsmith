  ...
  console.log(
    JSON.stringify(
      requests.filter((v) => {
        if (
          !v.response ||
          (v.response.statusCode >= 200 && v.response.statusCode < 300)
        ) {
          return false
        }
        return true
...
export const log = (message, group = '') =>
  console.log(
    '\n',
    '\x1b[32m',
    `${group ? `[${group}] ` : ''}${message}`,
...
export const assertTextContent = (selector, v) =>
  t.expect(Selector(selector).textContent).eql(v)
export const assertTextContentContains = (selector, v) =>
