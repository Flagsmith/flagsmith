...
export const testSegment1 = async (flagsmith: any) => {
  ...
  const thirdRule = segmentRules[3]
  await setSegmentRule(3, 0, thirdRule.name, thirdRule.operator, thirdRule.value + 1)
  await click(byId('update-segment'))
  await closeModal()
  await gotoSegments()
  await click(byId('segment-0-name'))
  await assertTextContent(byId(`rule-2-value-0`), `${thirdRule.value + 1}`)
  await closeModal()
}
