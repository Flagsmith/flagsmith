...
export default async () => {
    ...
    await addRequestHooks(versionApiLogger)
    ...
    await expect(versionApiLogger.requests.length).gte(1, 'Expected versioned API to be called')
    ...
    await expect(json.c.enabled).toBe(false)
    ...
    await expect(versionApiLogger.requests.length).gte(1, 'Expected versioned API to be called for toggle ON')
    ...
    await expect(json.c.enabled).toBe(true)
