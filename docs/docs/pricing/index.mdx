...
Self-hosted installations of Flagsmith do not have API request limits.
...
The following screenshot shows an example usage graph available from the Flagsmith dashboard:
...
API request. Requests to the [Admin API](/clients/rest#private-admin-api-endpoints) or requests made by Flagsmith SDKs
to track [Flag Analytics](/advanced-use/flag-analytics) are not billed.
