...
PII **must be** transient. Email addresses, phone numbers, full names, and any other identifying personal data should never be stored as persistent traits in Flagsmith.
...
To summarize, never persist PII in Flagsmith, neither as identifier nor as persistent trait.
