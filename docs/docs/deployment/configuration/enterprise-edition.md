...
(`flagsmith/flagsmith-api-ee` or `flagsmith/flagsmith-private-cloud`). The environment variables below have been split
into 2 separate tables. The first table includes those environment variables that are shared between the two images and
the second table includes those which take slightly different values.
