...
## Automated setup with provisioning

You can automate the Flagsmith-on-Flagsmith setup using
[Provisioning](/deployment-self-hosting/core-configuration/provisioning). A provisioning YAML file can create the
required project, environments, and flags in a single step — see the
[Flagsmith on Flagsmith provisioning guide](/deployment-self-hosting/core-configuration/provisioning#flagsmith-on-flagsmith)
