...
To implement your own offline handler, use the `BaseOfflineHandler` interface. It must return an
<a href={`https://www.tsdocs.dev/docs/flagsmith-nodejs/${NodejsVersion()}/classes/EnvironmentModel.html`}>EnvironmentModel</a>
object:

```typescript
import { BaseOfflineHandler } from 'flagsmith-nodejs';
...
<a href={`https://www.tsdocs.dev/docs/flagsmith-nodejs/${NodejsVersion()}/interfaces/FlagsmithCache.html`}>FlagsmithCache</a> interface.

For example, this cache implementation uses Redis as a backing store:

```typescript
import { Flags, Flagsmith, FlagsmithCache } from 'flagsmith-nodejs';
