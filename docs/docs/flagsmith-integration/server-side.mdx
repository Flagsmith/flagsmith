...
[reading up about the differences](/flagsmith-integration/integration-overview) first before integrating the SDKS into your applications.
...
<CodeBlock>
    {`<PackageReference Include="Flagsmith" Version="`}
    <DotnetVersion />
    {`" />`}
</CodeBlock>
...
### When running in [Remote Evaluation mode](/flagsmith-integration/integration-overview)
...
### When running in [Local Evaluation mode](/flagsmith-integration/integration-overview)
...
- _Only_ the traits provided to the SDK at runtime will be used. Local Evaluation mode, by design, does not make any
  network requests to the Flagsmith API when evaluating flags for an identity.
    - When running in Local Evaluation Mode, the SDK requests the
      [Environment Document](/flagsmith-integration/integration-overview) from the Flagsmith API. This contains all the
...
1. It can be used alongside [Offline Mode](server-side#offline-mode) to evaluate flags in environments with no network
   access
...
generate the [Environment Document](/flagsmith-integration/integration-overview) and use our LocalFileHandler class, but you can
...
to read an environment file generated by the [Flagsmith CLI](/flagsmith-integration/CLI):
...
When using Local Evaluation, it's important to read up on the [Pros, Cons and Caveats](/flagsmith-integration/integration-overview).
...
more details on initialising the SDK in offline mode.
