...
If your Mixpanel project is hosted in the EU or India data residency region, you must set the **Base URL** to the
matching regional API endpoint. By default, Flagsmith sends data to the US endpoint.
...
Mixpanel is sunsetting its legacy US→EU data forwarding in July 2026. After that date, data destined for EU or IN
projects that is sent to the US ingestion endpoint will be **permanently dropped**. Make sure you configure the correct
regional Base URL before that deadline.
