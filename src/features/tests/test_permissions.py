...
class FeaturePermissionsTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.group = UserPermissionGroup.objects.create(name='Test group', organisation=self.organisation)
    ...
    def test_project_user_with_read_access_can_list_features(self):
        # Given
        user_project_permission = UserProjectPermission.objects.create(user=self.user, admin=False,
    ...
    def test_project_admin_can_create_feature(self):
        ...
        UserPermissionGroupProjectPermission.objects.create(group=self.group, project=self.project, admin=True)
        mock_view.action = 'create'
        mock_view.detail = False
        mock_request.user = self.user
        mock_request.data = {
    ...
    def test_project_user_with_create_feature_permission_can_create_feature(self):
        # Given
        # use a group to test groups work too
        user_group_permission = UserPermissionGroupProjectPermission.objects.create(group=self.group,
                                                                                    project=self.project, admin=False)
    ...
    def test_organisation_admin_can_view_feature(self):
        ...
        # When
        result = feature_permissions.has_object_permission(mock_request, mock_view, self.feature)
    ...
    def test_project_user_with_view_project_permission_can_view_feature(self):
        # Given
        user_permission = UserProjectPermission.objects.create(user=self.user, project=self.project, admin=False)
