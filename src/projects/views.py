...
@method_decorator(name='list', decorator=swagger_auto_schema(manual_parameters=[
    openapi.Parameter('organisation', openapi.IN_QUERY,
                      'ID of the organisation to filter by.', required=False, type=openapi.TYPE_INTEGER)
]))
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        UserProjectPermission.objects.create(user=self.request.user, project=project, admin=True)
    ...
    def permissions(self, *args, **kwargs):
        return Response(PermissionModelSerializer(instance=ProjectPermissionModel.objects.all(), many=True).data)
    ...
    def user_permissions(self, request, *args, **kwargs):
        ...
        group_permissions = UserPermissionGroupProjectPermission.objects.filter(group__users=request.user,
                                                                                project=self.get_object())
        user_permissions = UserProjectPermission.objects.filter(user=request.user, project=self.get_object())
        ...
        data = {
            'admin': group_permissions.filter(admin=True).exists() or user_permissions.filter(admin=True).exists(),
...
class UserProjectPermissionsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return UserProjectPermission.objects.filter(project__pk=self.kwargs['project_pk'])
...
class UserPermissionGroupProjectPermissionsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return UserPermissionGroupProjectPermission.objects.filter(project__pk=self.kwargs['project_pk'])
