...
from projects.models import Project, UserProjectPermission, ProjectPermissionModel, UserPermissionGroupProjectPermission
...
class ProjectTestCase(TestCase):
    ...
    def test_should_create_a_project(self):
        ...
        # and user is admin
        assert UserProjectPermission.objects.filter(user=self.user, project__id=response.json()['id'], admin=True)
    ...
    def test_user_can_list_project_permission(self):
        ...
        assert len(response.json()) == 4  # hard code how many permissions we expect there to be
    ...
    def test_user_with_view_project_permission_can_view_project(self):
        ...
        user_project_permission = UserProjectPermission.objects.create(user=user, project=project)
    ...
    def test_user_with_view_project_permission_can_get_their_permissions_for_a_project(self):
        # Given
        user = FFAdminUser.objects.create(email='test@test.com')
        project = Project.objects.create(name='Test project', organisation=self.organisation)
...
class UserProjectPermissionsViewSetTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.user_project_permission = UserProjectPermission.objects.create(user=user, project=self.project)
        self.user_project_permission.permissions.set([read_permission])

        self.client = APIClient()
        self.client.force_authenticate(self.org_admin)

        self.list_url = reverse('api-v1:projects:project-user-permissions-list', args=[self.project.id])
        self.detail_url = reverse('api-v1:projects:project-user-permissions-detail',
    ...
    def test_user_can_create_new_user_permission_for_a_project(self):
        ...
        assert UserProjectPermission.objects.filter(user=new_user, project=self.project).exists()
        user_project_permission = UserProjectPermission.objects.get(user=new_user, project=self.project)
    ...
    def test_user_can_update_user_permission_for_a_project(self):
        ...
        assert 'CREATE_FEATURE' in self.user_project_permission.permissions.values_list('key', flat=True)
    ...
    def test_user_can_delete_user_permission_for_a_project(self):
        ...
        assert not UserProjectPermission.objects.filter(id=self.user_project_permission.id).exists()
...
class UserPermissionGroupProjectPermissionsViewSetTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.user_group_project_permission = UserPermissionGroupProjectPermission.objects.create(
        ...
        self.list_url = reverse('api-v1:projects:project-user-group-permissions-list', args=[self.project.id])
        self.detail_url = reverse('api-v1:projects:project-user-group-permissions-detail',
    ...
    def test_user_can_create_new_user_group_permission_for_a_project(self):
        ...
        assert UserPermissionGroupProjectPermission.objects.filter(group=new_group, project=self.project).exists()
        user_group_project_permission = UserPermissionGroupProjectPermission.objects.get(group=new_group,
                                                                                         project=self.project)
    ...
    def test_user_can_update_user_group_permission_for_a_project(self):
        ...
        assert 'CREATE_FEATURE' in self.user_group_project_permission.permissions.values_list('key', flat=True)
    ...
    def test_user_can_delete_user_permission_for_a_project(self):
        ...
        assert not UserPermissionGroupProjectPermission.objects.filter(
            id=self.user_group_project_permission.id).exists()
