...
class UserTestCase(TestCase):
    ...
    def test_join_organisation_alerts_admin_users_if_exceeds_plan_limit(self, MockThread):
        # Given
        Subscription.objects.create(organisation=self.organisation, max_seats=1)
        invite = Invite.objects.create(email=self.user.email, organisation=self.organisation)
        url = reverse('api-v1:users:user-join-organisation', args=[invite.hash])

        existing_org_user = FFAdminUser.objects.create(email='existing_org_user@example.com')
        ...
        # Then
        MockThread.assert_called_with(target=FFAdminUser.send_organisation_over_limit_alert, args=[self.organisation])
...
class UserPermissionGroupViewSetTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.regular_user = FFAdminUser.objects.create(email='user@testorganisation.com')
        self.regular_user.add_organisation(self.organisation, OrganisationRole.USER)

        self.list_url = reverse('api-v1:organisations:organisation-groups-list', args=[self.organisation.id])
    ...
    def _remove_users_url(self, permission_group_id: int) -> str:
        args = [self.organisation.id, permission_group_id]
        return reverse('api-v1:organisations:organisation-groups-remove-users', args=args)
    ...
    def test_regular_user_cannot_interact_with_groups(self):
        ...
        group = UserPermissionGroup.objects.create(name=group_name, organisation=self.organisation)
        data = {
            'name': 'New Test Group'
        }
    ...
    def test_can_add_multiple_users_including_current_user(self):
        # Given
        group = UserPermissionGroup.objects.create(name='Test Group', organisation=self.organisation)
        url = self._add_users_url(group.id)
        data = {
            'user_ids': [self.admin.id, self.regular_user.id]
        }

        # When
        response = self.admin_user_client.post(url, data=json.dumps(data), content_type='application/json')

        # Then
        assert response.status_code == status.HTTP_200_OK
        assert all(user in group.users.all() for user in [self.admin, self.regular_user])
    ...
    def test_cannot_add_same_user_twice(self):
        ...
        # When
        self.admin_user_client.post(url, data=json.dumps(data), content_type='application/json')
