...
class SegmentTestCase(TestCase):
    ...
    def test_percentage_value_for_given_segment_rule_and_identity_is_number_between_0_and_1(
        self,
    ):
        # Given
    ...
    def test_percentage_value_for_given_segment_rule_and_identity_is_the_same_each_time(
        self,
    ):
        # Given
        segment = Segment.objects.create(name="Test Segment", project=self.project)
    ...
    def test_percentage_value_is_unique_for_different_identities(self):
        ...
        another_identity = Identity.objects.create(
            environment=self.environment, identifier="another_test_identity"
        )
    ...
    def test_percentage_values_should_be_evenly_distributed(self):
        ...
        for j in range(num_test_buckets):
            ...
            bucket_value_limit = min(
                (j + 1) / num_test_buckets
                + error_factor * ((j + 1) / num_test_buckets),
                1,
            )
...
class ConditionTest(TestCase):
    ...
    def test_percentage_split_calculation_divides_value_by_100_before_comparison(
        self, get_identity_percentage_value
    ):
        # Given
