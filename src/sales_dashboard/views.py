...
from analytics.influxdb_wrapper import get_event_list_for_organisation, get_events_for_organisation
...
class OrganisationList(ListView):
    ...
    def get_queryset(self):
        if 'search' in self.request.GET:
            ...
            organisations = Organisation.objects.annotate(projects_num=Count('projects')).annotate(
                user_num=Count('users')).all().filter(name__icontains=search_term)
        else:
            organisations = Organisation.objects.annotate(projects_num=Count('projects')).annotate(
                user_num=Count('users')).all()
        ...
        for organisation in organisations:
            list_of_organisations.append({
                ...
                'flags': sum([project.features.count() for project in organisation.projects.all()]),
                'segments': sum([project.segments.count() for project in organisation.projects.all()]),
            })
