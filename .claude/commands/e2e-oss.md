...
If an argument is provided (e.g., `/e2e-oss 5`), it sets `E2E_REPEAT` to run tests multiple times to detect flakiness.
...
1. **RUN TESTS NOW** from the frontend directory:
   ```bash
   cd frontend
   E2E_RETRIES=0 SKIP_BUNDLE=1 E2E_CONCURRENCY=20 E2E_REPEAT=${ARGUMENTS:-0} npm run test -- --grep-invert @enterprise --quiet
   ```
