...
class IdentityViewSetDyanmoTestCase(TestCase):
    identifier = "user1"
    ...
    def setUp(self):
        ...
        self.identifier = "dynamo_test_user123"
        api_key = self.environment.api_key
        self.identity_dict = {
            "composite_key": f"{api_key}_{self.identifier}",
    ...
    def test_get_identity_calls_get_item(self, dynamo_identity_table):
        # Given - set up data
        dynamo_identity_table.get_item.return_value = {"Item": self.identity_dict}
        # When
        response = self.client.get(
            self.identities_url % (self.environment.api_key, self.identifier)
        )
        # Then
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["identifier"] == self.identifier
        dynamo_identity_table.get_item.assert_called_with(
            Key={"composite_key": f"{self.environment.api_key}_{self.identifier}"}
        )

    @mock.patch("environments.identities.models.dynamo_identity_table")
    def test_delete_identity_calls_delete_item(self, dynamo_identity_table):
        # Given
        dynamo_identity_table.get_item.return_value = {"Item": self.identity_dict}
        url = reverse(
            "api-v1:environments:environment-identities-detail",
            args=[self.environment.api_key, self.identifier],
        )
    ...
    def test_search_identities_calls_query_with_correct_arguments(
        ...
        dynamo_identity_table.query.assert_called_with(
            ...
            & Key("identifier").begins_with(self.identifier),
        )
    ...
    def test_search_for_identities_with_exact_match_calls_query_with_correct_argument(
        ...
        url = "%s?%s" % (
            base_url,
            urllib.parse.urlencode({"q": f'"{self.identifier}"'}),
        )
        ...
        dynamo_identity_table.query.assert_called_with(
            ...
            & Key("identifier").eq(self.identifier),
        )
