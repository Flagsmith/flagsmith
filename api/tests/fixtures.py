"""
Pytest fixtures for the Flagsmith API test suite.

These fixtures were originally defined in `api/conftest.py`. They were extracted
into this module so that downstream consumers that install the api package can
re-export them from their own `conftest.py` via:

    from tests.fixtures import *  # noqa: F401, F403

without colliding with the consumer's own root `conftest.py`.

The api's own `api/conftest.py` re-exports everything from this module so that
running pytest from `api/` still discovers every fixture.
"""
