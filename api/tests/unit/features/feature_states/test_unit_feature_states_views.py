...
def test_delete_segment_override__existing_override__removes_override(
    ...
    version = None
    if environment_.use_v2_feature_versioning:
        version = get_current_live_environment_feature_version(
            environment_.id, feature.id
        )
...
def test_delete_segment_override__segment_not_in_project__returns_400(
    ...
    # Segment doesn't exist in project
    data = {
        "feature": {"name": feature.name},
        "segment": {"id": 999999},
    }
...
def test_delete_segment_override__no_existing_override__returns_404(
    ...
    # Segment exists in project but has no override
    segment = Segment.objects.create(name="segment_no_override", project=project)
...
def test_delete_segment_override__workflow_enabled__returns_403(
    ...
    # Given
    with_environment_permissions([UPDATE_FEATURE_STATE])  # type: ignore[call-arg]
    environment_.minimum_change_request_approvals = 1
    environment_.save()
...
def test_delete_segment_override__v2_versioning__creates_new_version(
    staff_client: APIClient,
    feature: Feature,
    environment_v2_versioning: Environment,
    project: Project,
