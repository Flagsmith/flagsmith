...
def test_create_feature_serializer__get_identity_feature_state__returns_state_from_context() -> (
    None
):
    # Given
    ...
    with mock.patch(
        "features.serializers.FeatureStateSerializerSmall"
    ) as MockSerializer:
        ...
        context: dict[str, Any] = {"identity_feature_states": {1: feature_state}}
...
def test_create_feature_serializer__get_identity_feature_state__returns_none_if_no_state_in_context() -> (
    None
):
    # Given
    feature = mock.MagicMock(spec=Feature, id=1)

    context: dict[str, Any] = {"identity_feature_states": {}}
    serializer = CreateFeatureSerializer(instance=feature, context=context)
...
def test_create_feature_serializer__get_identity_feature_state__returns_none_if_context_missing_key() -> (
    None
):
    # Given
    feature = mock.MagicMock(spec=Feature, id=1)
