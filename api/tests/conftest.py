"""
Pytest hooks and fixtures for the Flagsmith API test suite.

This module is the single source of truth for the api's test hooks and
fixtures. It is loaded in two ways:

1. When pytest is invoked from ``api/``, pytest's rootdir-to-test conftest
   walk discovers this file as the conftest for everything under ``tests/``.

2. Downstream consumers that install ``flagsmith-api[dev]`` opt in to the
   same fixtures by declaring the following in their rootdir conftest::

       pytest_plugins = ["tests.conftest"]

   Pytest's ``pytest_plugins`` mechanism loads plugins after
   pytest-django's ``pytest_load_initial_conftests`` hook
   (``tryfirst=True``) has configured Django, so the module-level Django
   imports below are safe.

   Refs https://github.com/pytest-dev/pytest/issues/935 for why an
   entry-point (``pytest11``) plugin would NOT work here -- entry-point
   plugins are imported before pytest-django sets up Django.
"""
