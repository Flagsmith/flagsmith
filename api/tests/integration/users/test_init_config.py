...
def test_returns_404_when_user_exists(admin_user, django_client):  # type: ignore[no-untyped-def]  # noqa: FT003
    # Given
...
def test_returns_200_when_no_user_exists(django_client):  # type: ignore[no-untyped-def]  # noqa: FT003
    # Given
    url = reverse("api-v1:users:config-init")
...
def test_valid_request_creates_admin_and_updates_site(db, django_client):  # type: ignore[no-untyped-def]  # noqa: FT003
    # Given
    url = reverse("api-v1:users:config-init")
    form_data = {
        "username": "test-admin",
...
def test_invalid_form_does_not_change_anything(db, django_client):  # type: ignore[no-untyped-def]  # noqa: FT003
    # Given
    url = reverse("api-v1:users:config-init")
    form_data = {
        "username": "test-admin",
