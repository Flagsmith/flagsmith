...
def test_config_init__user_exists__returns_404(admin_user, django_client):  # type: ignore[no-untyped-def]
    # Given
...
def test_config_init__no_user_exists__returns_200(django_client):  # type: ignore[no-untyped-def]
    # Given
    url = reverse("api-v1:users:config-init")
...
def test_config_init__valid_request__creates_admin_and_updates_site(db, django_client):  # type: ignore[no-untyped-def]
    # Given
    url = reverse("api-v1:users:config-init")
    form_data = {
        "username": "test-admin",
...
def test_config_init__invalid_form__does_not_change_anything(db, django_client):  # type: ignore[no-untyped-def]
    # Given
    url = reverse("api-v1:users:config-init")
    form_data = {
        "username": "test-admin",
