...
def migrate_feature_segments_forward(apps, schema_editor):  # type: ignore[no-untyped-def]
    ...
    for feature_segment in FeatureSegment.objects.all():
        for idx, environment in enumerate(
            feature_segment.feature.project.environments.all()
        ):
            ...
            if idx == 0:
                logger.info(
                    "Adding environment %d to feature segment %d"
                    % (environment.id, feature_segment.id)
                )
                feature_segment.environment = environment
                feature_segment.save()
            else:
                logger.info(
                    "Creating new feature segment for feature %d, environment %d and segment %d"
                    % (
                        feature_segment.feature.id,
                        environment.id,
                        feature_segment.segment.id,
                    )
                )
                ...
                FeatureState.objects.filter(
                    environment=environment,
                    feature=new_feature_segment.feature,
...
def migrate_feature_segments_reverse(apps, schema_editor):  # type: ignore[no-untyped-def]
    ...
    for feature in Feature.objects.filter(
        feature_segments__isnull=False
    ).prefetch_related("feature_segments"):
        ...
        FeatureSegment.objects.filter(feature=feature).exclude(
            pk=first_feature_segment.pk
        ).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            migrate_feature_segments_forward,
            reverse_code=migrate_feature_segments_reverse,
        ),
    ]
