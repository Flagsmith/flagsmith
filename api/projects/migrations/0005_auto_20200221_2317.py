...
def create_default_project_admins(apps, schema_editor):  # type: ignore[no-untyped-def]
    ...
    for user_organisation in UserOrganisation.objects.filter(
        ...
        for project in user_organisation.organisation.projects.all():
            UserProjectPermission.objects.create(
                project=project, user=user_organisation.user, admin=True
            )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            create_default_project_admins, reverse_code=lambda *args: None
        )
    ]
