...
def map_segment_to_segment_context(
    ...
    *,
    ...
    Rules are read from the `SegmentRule` and `Condition` rows, which the
    segment caches prefetch for this. `Segment.rules_data` holds the same tree
    as stored JSON and would save those queries, but is not yet written by
    every path that creates a segment — a cohort's is not.
    TODO: Read the rules from `rules_data`, as per
    https://github.com/Flagsmith/flagsmith/issues/7816
    """
...
def map_identity_overrides_to_segment_context(
    ...
    return {
        ...
        "overrides": overrides,
        # Deliberately no metadata: it is not a segment anyone can name, and
        # its absence is what tells a caller reading `$.segments` to skip it.
    }
